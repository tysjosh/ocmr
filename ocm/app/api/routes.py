...
1. ``POST /memory/write`` — run the write pipeline (W1–W8).
2. ``POST /memory/query`` — run the retrieval pipeline (R0–R4).
3. ``POST /memory/validate`` — validate a candidate without committing.
4. ``GET /memory/entity/{id}`` — fetch an entity and its assertions.
5. ``GET /memory/conflicts`` — list unresolved conflicts / quarantines.
...
def write_memory(
    ...
    mirror :class:`~ocm.memory.write_pipeline.WriteResult`.
    """
...
def query_memory(
    ...
    list. The pipeline classifies internally, but we classify once
...
def validate_candidate(
    ...
    """Validate a candidate (W5→W6→W7) **without committing** anything.
...
def get_conflicts(c: CoreContainer = Depends(get_container)) -> ConflictsResponse:
    """Return unresolved conflicts and quarantined candidates.
