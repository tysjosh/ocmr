...
* ``QuarantineStore.add/list/set_status`` round-trips through the
  ...
  same repository (the restart guarantee).
* ``ProvenanceTracker.record/for_subject`` round-trips through the
  ``provenance`` table, recording ``source_ref`` / ``created_at`` /
  ``extractor_version`` / ``supporting_evidence_ids``, and preserves
  multiple provenance rows per subject (supersession case).
"""
...
def test_quarantine_persists_across_restarts(repo, ids):
    """A fresh store on the same repo sees previously persisted records."""
...
def test_provenance_preserves_multiple_records_per_subject(repo, ids):
    """Supersession keeps provenance for both old and new."""
    tracker = ProvenanceTracker(repo, ids)
